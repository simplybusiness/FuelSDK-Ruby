...
module FuelSDK
	...
	include FuelSDK::Rest
		...
		def jwt= encoded_jwt
			...
			self.auth_token = decoded_jwt.first['request']['user']['oauthToken']
			self.internal_token = decoded_jwt.first['request']['user']['internalOauthToken']
			self.refresh_token = decoded_jwt.first['request']['user']['refreshToken']
			self.auth_token_expiration = Time.new + decoded_jwt.first['request']['user']['expiresIn']
			self.package_name = decoded_jwt.first['request']['application']['package']
		end
